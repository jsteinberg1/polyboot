...
# ========
# A simple program to factory reset Polycom phones using CURL commands to interact with the phone's web UI.
#
#
# Usage
# ~~~~~
...
def factory_reset(ip,admin_password):
    factory_curl = ['curl',
                    '-k',
...
with open('iplist.txt') as f:
    ...
    for IP_ADDR, Password in reader:
        ...
        factory_reset(ip,admin_password)
        print(f'Factory reset instruction sent to address {ip} using password {Passwd}')